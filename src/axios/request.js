  ...
  response => {  //成功请求到数据
    ...
    if(response.config.responseType === 'blob') {  /** 此处代码是为了做二进制流的下载判断*/
      ...
      if(response.config.method === 'get') {
        ...
        if(response.config.url === '/api/project/export/engineeringTaskBookExport') {
          if(response.config.args.contractNo) {
            filename = `工程任务书${response.config.args.contractNo}-${vm.$format(new Date())}.xlsx`;
          }else {
            filename = `工程任务书-${vm.$format(new Date())}.xlsx`;
          }
        }
        if(response.config.url === '/api/getPmSystemItemPart/export/processDetailsExport' && response.config.args.mouldNo) {
          filename = `加工进度计划表${response.config.args.mouldNo}-${vm.$format(new Date())}.xlsx`;
        }
        if(response.config.url === '/api/sale/export/salesPlanDetailsExport') {
          filename = `销售计划表-${vm.$format(new Date())}.xlsx`;
        }
      }else{
