...
//搜索按钮调用接口数据
      async search() {
        ...
        console.log(this.mouldNoDisabled);
      },
            ...
            if(form.mouldNo && form.goodsType && form.transportType && form.transportCompany) {
              if((form.goodsType === 1 && form.sendSampleCount) || (form.goodsType === 2 && !form.sendSampleCount)) {
                // if (form.positionInfo.length) {
                  ...
                  this.flag_add = 1;
                  if(res.status === 1) {
                    let params = {page:this.currentPage, size: this.size, mark: 1}, mould = this.$store.getters.itemCheck_item.mouldNo;
                    if(mould) {
                      params.mouldNo = mould;
                    }
