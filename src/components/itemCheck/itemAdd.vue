...
<script>
  ...
  import {getProjectDescribeList,getUserListByRoleType,getMouldBasicInfo,getProjectNodeManageList,getWarnList,
    getProjectStatusList,getProjectStrageList,addProjectNodeManage,updateProjectInfo,updateProjectInfoDetail,addOrUpdateProjectDescribe,} from '../../axios/api.js'
  export default {
    name: "itemAdd",
...
//根据模号获取项目基本信息
      async getMouldBasicInfo(arg) {
        ...
        if(mould !== undefined && mould !== null && mould !== '') {
          let res = await getMouldBasicInfo({mouldNo:mould,mark: this.$store.getters.itemCheck_item.mark});
...
//新增项目或编辑项目
      newItem() {
        this.$refs.itemAdd.validate(async (valid) => {
          if (valid) {
            if(this.basicInfo.mouldNo) {
              let params = {...this.project, mark: this.$store.getters.itemCheck_item.mark};
              //判断是编辑还是新增
              if(this.id) {
                params.id = this.id;
              }
              ...
              //   let res = await updateProjectInfo(params);
              //   if(res.status === 1) {
            ...
            //不判断是否提醒必填
              ...
              let res;
              if(params.mark === 2) {
                res = await updateProjectInfo(params);
              } else {
                res = await updateProjectInfoDetail(params);
              }
              if(res.status === 1) {
                this.$message({showClose: true, type: 'success', message: '创建或保存成功！'});
                this.$store.dispatch('itemCheck_item',{});
                if(params.mark === 2) {
                  this.$router.push('itemCheck');
                } else {
                  this.$router.push('projectsPointManage');
                }
                // this.$goback();
              }
            }else {
